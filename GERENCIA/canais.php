	...
	<script>
		...
		function importarContatosAtivos(id,tk,ret){
			...
			$.ajax({
				...
				type: 'POST',
				dataType: 'json',
				data: {ID: id,TOKEN:tk,IDREFCANAL:ret},
			})
			.done(function(response) {
		...
		while ($row=mysqli_fetch_assoc($execute)) {
			...
			<div class="col-sm-4 jumbotron">
				...
				<div class="card-body">
					...
					onclick="novaconexao('<?=$row['id_canalzapio']?>',
					'<?=$row['token_canalzapio']?>','<?=$row['idcanalzapio']?>')" id='btn_<?=$row['idcanalzapio']?>' id='btnatualizar'>
					Atualizar
				</button>
				<br>
				<button class="btn btn-danger btn-block desligar" 
				onclick="desligar('<?=$row['id_canalzapio']?>',
				'<?=$row['token_canalzapio']?>','<?=$row['idcanalzapio']?>')" id='btn_desligar<?=$row['idcanalzapio']?>'>
				DESLIGAR
			</button>
			<br>
<!-- 
