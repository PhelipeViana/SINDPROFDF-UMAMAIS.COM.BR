<?php
$TIPO = $_REQUEST['p1'];
$ID1 = $_REQUEST['p2']; //LINHA
$ID2 = $_REQUEST['p3']; //COLUNA
...
if ($TIPO == 1) {
	...
	$sql_create_campanha = "INSERT INTO `campanha_envios` (
		...
		msg,
		`create_campanha`) 
	VALUES ('$nomecamp','CAMPANHA $nomecamp','$create_campanha')";

	$exe_create = mysqli_query($conn, $sql_create_campanha);
	$ID_CAMPANHA = mysqli_insert_id($conn);
	//fim criar campanha

	$subquery = "SELECT distinct REF_CADASTRO FROM `hast_cadastro` WHERE `REF_HASTAG`='$ID1'";
	...
	$sub = "SELECT distinct REF_CADASTRO FROM `hast_cadastro` WHERE `REF_HASTAG`='$ID1'";
	$subquery = "SELECT distinct REF_CADASTRO FROM `hast_cadastro` WHERE 
	REF_CADASTRO IN ($sub)
	AND
	`REF_HASTAG`='$ID2'";
	...
	echo json_encode(['numberssss' => $cont, 'st' => 1, 'id_camp' => $ID_CAMPANHA]);
	$sql_alter = "UPDATE `campanha_envios` SET msg='$nomecamp',`num_envio`='$cont' WHERE `id_cam_envio`='$ID_CAMPANHA'";
	$e_alter = mysqli_query($conn, $sql_alter);
}
