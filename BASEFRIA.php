...
<!-- 			<label for="">LINKS DE RESPOSTA</label>
			...
			<table class="table">
				...
				<tbody>
					...
					$sql="SELECT * FROM `DISPAROS` WHERE `enviado`=0 limit 50";
					...
					while ($r=mysqli_fetch_assoc($e)) {
						...
						<tr>
							...
							<td><?=$r['num_disparo'];?></td>
							<td class='resultado'
							data-nome='<?=$r['nome_disparo'];?>'
							data-numero='<?=$r['num_disparo'];?>'
							data-id="<?=$ROW_CANAL['id_canalzapio']?>"
							data-tk="<?=$ROW_CANAL['token_canalzapio']?>"
							data-reflist='<?=$r['id_disparos'];?>'
							>NA FILA</td>
	...
	$sql_enviados="SELECT * FROM `DISPAROS` WHERE `enviado` > 0";
	$e=mysqli_query($conn,$sql_enviados);
	$num_enviados=mysqli_num_rows($e);


	$sql_nao="SELECT * FROM `DISPAROS` WHERE `enviado` =0";
